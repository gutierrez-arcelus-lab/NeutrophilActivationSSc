---
title: 'Differential Gene Expression in Systemic Sclerosis: GSE181549'
...
# 1) Download GEO GSE181549 series matrix ----
```{r}
gse_id <- "GSE181549"
...
```{r}
ph_summ <- ph %>%
  select(characteristics_ch1, characteristics_ch1.2, characteristics_ch1.3,characteristics_ch1.4,characteristics_ch1.7) %>%
  group_by(characteristics_ch1,characteristics_ch1.7) %>%
...
```{r}
# There is a break in the columns distribution between cases and controls and inside the cases. We need to break into pieces the df and rebuilt.
ph_ctrl <- ph %>% filter(characteristics_ch1 == 'condition: Control')
ph_ssc <- ph %>% 
  filter(characteristics_ch1 != 'condition: Control') %>%
  filter(characteristics_ch1.2 == 'sample number within subject: 1') %>%
  filter(characteristics_ch1.7 == 'diffuse at baseline sample: Diffuse') 
toKeep <- rbind(ph_ctrl,ph_ssc)
ph <- ph %>% filter(title %in% toKeep$title)
```
...
```{r}
ph$Group <- case_when(
  grepl("condition: Control", ph$characteristics_ch1, ignore.case = TRUE) ~ "Control",
  TRUE ~ "dcSSc"
)
...
ph <- ph %>%
  mutate(
    # Extract AGE from whichever column it appears in
    Age_raw = case_when(
      str_detect(characteristics_ch1.2, "age:") ~ characteristics_ch1.2,
      str_detect(characteristics_ch1.3, "age:") ~ characteristics_ch1.3,
      TRUE ~ NA_character_
    ),
    Age = as.numeric(str_extract(Age_raw, "\\d+")),  # extract numeric age

    # Extract SEX
    Sex_raw = case_when(
      str_detect(characteristics_ch1.3, "gender:") ~ characteristics_ch1.3,
      str_detect(characteristics_ch1.4, "gender:") ~ characteristics_ch1.4,
      TRUE ~ NA_character_
    ),
    Sex = str_replace(Sex_raw, ".*gender\\s*[:=]\\s*([A-Za-z]+).*", "\\1"),

    # Extract ETHNICITY
    Ethnicity_raw = case_when(
      str_detect(characteristics_ch1.4, "race:") ~ characteristics_ch1.4,
      str_detect(characteristics_ch1.5, "race:") ~ characteristics_ch1.5,
      TRUE ~ NA_character_
    ),
    Ethnicity = str_replace(Ethnicity_raw, ".*race\\s*[:=]\\s*([A-Za-z]+).*", "\\1")
  )

# Check results
ph %>% select(title, Group, Age, Sex, Ethnicity) %>% head()
```
...
```{r}
boxplot(E, las = 2, main = "Pre-norm", col = "grey90")
```
...
range(E, na.rm = TRUE)
```
Conclusion: values are log2+norm, no action require
```{r}
...
```{r}
#boxplot(E, las = 2, main = "Post log2 + quantile normalization", col = "grey90")
```
...
```{r}
E_raw <- E
E <- E[, rownames(ph), drop = FALSE]
pc_dat <- t(E)  # samples x genes
pc_dat[is.na(pc_dat)] <- 0
pca <- prcomp(pc_dat, scale. = TRUE)
pc_df <- data.frame(pca$x[, 1:2], Group = ph$Group, Sample = rownames(pc_dat),age  = ph$Age, ethnicity  = ph$Ethnicity, sex = ph$Sex, SampleID = ph$title)
#pc_df$age <- round(as.numeric(pc_df$age),0)
```
...
```{r}
ggplot(pc_df, aes(PC1, PC2, color = age, label = Sample)) +
  geom_point(size = 2) +
  theme_bw() +
  guides(color = guide_legend(override.aes = list(size = 3)))
```
...
```
```{r}
ggplot(pc_df, aes(PC1, PC2, color = ethnicity, label = Sample)) +
  geom_point(size = 2) +
  theme_bw() 
```

```{r}
ggplot(pc_df, aes(PC1, PC2, color = sex, label = Sample)) +
  geom_point(size = 2) +
  theme_bw() 
```
...
fit <- lmFit(E, design)
contrast.matrix <- makeContrasts(SSc_vs_Control = dcSSc - Control, levels = design)
...
gene <- map_df %>% filter(SYMBOL == 'COL4A4')
