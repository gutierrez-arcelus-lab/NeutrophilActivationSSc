---
title: 'Differential Gene Expression in Systemic Sclerosis: GSE48149'
...
# 1) Download GEO GSE48149 series matrix ----
```{r}
gse_id <- "GSE48149"
...
```{r}
DT::datatable(pData(eset))
keep <- !grepl("IPF|IPAH|PPH|SSc-PAH", ph$source_name_ch1, ignore.case = TRUE)
eset <- eset[, keep]
ph <- pData(eset)
...
```{r}
ph$Group <- case_when(
  grepl("NL", ph$source_name_ch1, ignore.case = TRUE) ~ "Control",
  grepl("ssc", ph$source_name_ch1, ignore.case = TRUE) ~ "SSc",
  TRUE ~ "Other"
)
ph$Condition <- case_when(
  grepl("NL", ph$source_name_ch1, ignore.case = TRUE) ~ "Control",
  grepl("PF", ph$source_name_ch1, ignore.case = TRUE) ~ "SSc-PF",
  #grepl("PAH", ph$source_name_ch1, ignore.case = TRUE) ~ "SSc-PAH",
  TRUE ~ "Other"
)
```
...
pc_df <- data.frame(pca$x[, 1:2], Group = ph$Group, Sample = rownames(pc_dat),Condition  = ph$Condition, SampleID = ph$title)
```
...
gene <- map_df %>% filter(SYMBOL == 'KRT17')
