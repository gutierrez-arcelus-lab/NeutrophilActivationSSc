---
title: 'Differential Gene Expression in Systemic Sclerosis: GSE95065'
...
# 1) Download GEO GSE95065 series matrix ----
```{r}
gse_id <- "GSE95065"
...
```{r}
ph_summ <- ph %>%
  select(characteristics_ch1.1,characteristics_ch1.2) %>%
  group_by(characteristics_ch1.1,characteristics_ch1.2) %>%
...
```{r} 
# set at the end, as everytime I modifed the table it get rebooted and mess with the IDEL
#DT::datatable(pData(eset), options = list(pageLength = 5), elementId = "meta_table")
```

# Create a clean group label:
```{r}
ph$Group <- case_when(
  grepl("healthy", ph$`dx:ch1`, ignore.case = TRUE) ~ "Control",
  TRUE ~ "dcSSc"
)
ph$batch <- ph$`batch:ch1`
```
...
```{r}
# As the matrix is not normalize--values higher than 100, I will do norm
...
```{r}
# As this is an old array with "_at" probes structure, we need to adjust the approach
# 1) Ensure ENTREZ_GENE_ID is character and remove empty entries
tbl$ENTREZ_GENE_ID <- as.character(tbl$ENTREZ_GENE_ID)
tbl <- tbl[!is.na(tbl$ENTREZ_GENE_ID) & tbl$ENTREZ_GENE_ID != "", ]

# 2) Mapping ENTREZ IDs to official gene symbols using org.Hs.eg.db
map_df <- AnnotationDbi::select(
  org.Hs.eg.db,
  keys = unique(tbl$ENTREZ_GENE_ID),
  columns = c("SYMBOL"),
  keytype = "ENTREZID"
)

# 3) Merging mapping results with the original platform table
tbl_merged <- left_join(tbl, map_df, by = c("ENTREZ_GENE_ID" = "ENTREZID"))
head(tbl_merged[, c("ID", "ENTREZ_GENE_ID", "SYMBOL", "Description")])
```

```{r}
# 4) Building the final mapping table (PROBEID → SYMBOL)
map_final <- tbl_merged %>%
  dplyr::select(PROBEID = ID, SYMBOL) %>%
  dplyr::filter(!is.na(SYMBOL)) %>%
  dplyr::distinct()

# 5) Aligning the mapping table with the expression matrix
E <- exprs(eset)
E <- E[rownames(E) %in% map_final$PROBEID, , drop = FALSE]
map_final <- map_final[match(rownames(E), map_final$PROBEID), ]
rownames(E) <- map_final$SYMBOL

# Print summary
cat("Mapped genes (unique):", nrow(E), "\n")
```
...
pc_df <- data.frame(pca$x[, 1:2], Group = ph$Group, Sample = rownames(pc_dat),SampleID = ph$title,batch=ph$batch)
```
...
```{r}
ggplot(pc_df, aes(PC1, PC2, color = batch, label = Sample)) +
  geom_point(size = 2) +
  theme_bw() +
  guides(color = guide_legend(override.aes = list(size = 3)))
```
...
```{r}
design <- model.matrix(~ 0 + Group + batch, data = ph) # adding batch as we are observing a batch effect
fit <- lmFit(E, design)
contrast.matrix <- makeContrasts(SSc_vs_Control = GroupdcSSc - GroupControl, levels = design)
...
#t <- merge(tt,map_final, by.x="X",by.y="SYMBOL")
t <- tt
```
...
gene <- map_final %>% filter(SYMBOL == 'COL4A4')
geneName <- unique(gene$SYMBOL)
E_fil <- as.data.frame(E) %>% filter(rownames(E) == gene$SYMBOL) %>% t() %>% as.data.frame()
colnames(E_fil) <- "gene"
df <- cbind(E_fil,ph)
