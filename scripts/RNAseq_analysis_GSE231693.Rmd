---
title: 'Differential Gene Expression in Systemic Sclerosis: GSE231693'
...
# 1) Download GEO GSE231693 series matrix
```{r}
gse_id <- "GSE231693"
...
```{r}
ph <- ph %>% filter(`disease state:ch1` != 'idiopathic pulmonary fibrosis')
```
...
```{r}
counts <- read_tsv("../data/GSE231693_raw_counts_GRCh38.p13_NCBI.tsv")
```
...
```
It seems that there are 3 samples missing from the raw file

```{r}
setdiff(samples_s, colnames(counts_mat))
```

```{r}
setdiff(colnames(counts_mat), samples_s)
```
...
print(count_summary)
```
We need to filter ph file too so we don't have erros downstream
```{r}
ph <- ph %>% filter(geo_accession %in% colnames(counts_mat))
```
...
volplot <- ggplot(res2_ann, aes(x = logFC, y = -log2(adj.P.Val), color= significance,label = delabel)) +
  ...
  geom_label_repel(data = filter(res2_ann, !is.na(delabel)),, color = "black",
                   max.overlaps = 40, force = 5, size = 3, show.legend = FALSE) +
...
Run GSEA
```{r GSEA}
# changing some parameters to avoid NAs
gsea.c2 <- fgseaMultilevel(
  pathways = react,
  stats = ranking,
  minSize = 10,
  maxSize = 500,
...
neu_terms
```
For some, Neutrophil Degranulation pathway doesn't have p-value. ES < 0.
...
“There were X pathways for which P-values were not calculated properly due to unbalanced (positive and negative) gene-level statistic values. For such pathways pval, padj, NES, log2err are set to NA.”
