---
title: 'Differential Gene Expression in Systemic Sclerosis: GSE130955'
...
# 1) Download GEO GSE130955 series matrix ----
```{r}
gse_id <- "GSE130955"
...
```{r}
ph_summ <- ph %>%
  select(characteristics_ch1.1, characteristics_ch1.2, characteristics_ch1.3, characteristics_ch1.4, characteristics_ch1.5) %>%
...
```{r}
ph <- ph %>% filter(characteristics_ch1.2=='biopsy number for this subject: 1')
...
```{r}
ph$Group <- case_when(
  grepl("diagnosis: healthy control", ph$characteristics_ch1.1, ignore.case = TRUE) ~ "Control",
  grepl("diagnosis: SSc patient", ph$characteristics_ch1.1, ignore.case = TRUE) ~ "SSc",
  TRUE ~ "Other"
)

ph$sex <- case_when(
  grepl("1", ph$`sex (0 = female, 1 = male):ch1`, ignore.case = TRUE) ~ "male",
  grepl("0", ph$`sex (0 = female, 1 = male):ch1`, ignore.case = TRUE) ~ "female",
  TRUE ~ "Other"
)

```

# 3) Extracting the count matrix from local 
```{r}
counts <- read_tsv("../data/GSE130955_raw_counts_GRCh38.p13_NCBI.tsv")
```
...
volplot <- ggplot(res2_ann, aes(x = logFC, y = -log2(adj.P.Val), color= significance,label = delabel)) +
  ...
  geom_label_repel(data = filter(res2_ann, !is.na(delabel)),, color = "black",
                   max.overlaps = 20, force = 5, size = 3, show.legend = FALSE) +
...
volplot
```
The quality of this dataset is too low for us to do a reliable DEA and get any result.

```{r, fig.height= 4, fig.width= 5}
# Box plot as sanity check
gene <- map %>% filter(SYMBOL == 'CD93')
...
```{r, fig.height= 10, fig.width= 18}
  gsea.c2.plot <- ggplot(top_combined, aes(reorder(pathway, NES), NES)) +
    ...
    coord_flip() +
    labs(x = "Pathway", y = "Normalized Enrichment Score",
         title = paste0("REACTOME top 20 pathways in ",gse_id)) + 
    theme_bw() +
    theme(
      axis.text.x = element_text(size = 12),
      axis.text.y = element_text(size = 12,face = "bold"),
